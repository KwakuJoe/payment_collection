<template>
    <div class="flex flex-col w-full p-5 border border-gray-100 gap-y-5 dark:border-zinc-800">
        ...
        <div class="flex flex-col items-end lg:flex-row gap-x-5 gap-y-2">
            <div class="flex flex-col w-full lg:w-2/4 ">
                ...
                <div class="flex flex-col items-start w-full gap-y-2">
                    ...
                    <InputText size="small" fluid v-model="verifyBankTransferPaymentAccountPayload.account_number"
                        placeholder="Account Number" autocomplete="off" v-bind:oninput="make_input_number" />
                </div>
            ...
            <div class="flex flex-col w-full lg:w-1/4 ">
                ...
                <div class="flex flex-col items-start w-full gap-y-2">
                    ...
                    <InputText size="small" fluid
                        v-model="prepareFormFields[`${paymentStore.selectedPaymentServiceFormFieldIsAmount![0]?.field_name}`]"
                        variant="filled" class="bg-gray-500 border-2" :readonly="true" autocomplete="off"
                        v-bind:oninput="make_input_number" disabled />
                </div>

            </div>
            <!-- verify button -->
            <div class="flex items-center w-full lg:w-1/4">
                <Button size="small" @click="verifyBankTransferPaymentAccount()" :loading="isVerificationLoading"
                    variant="outline_primary" label="Verify" class="w-full " />
            </div>
        </div>
        ...
        <div v-if="sufficient_account"
            ...
            <div class="flex flex-col flex-1">
                <p class="text-sm font-semibold text-green-500">
                    {{ verifyBankTransferPaymentAccountPayload.account_number }}</p>
        ...
        <div v-if="insufficient_account"
            class="flex flex-wrap items-center w-full p-5 border rounded-md gap-x-5 gap-y-5 bg-red-500/5 border-red-500/15">
            ...
            <div class="flex flex-col flex-1">
                <p class="text-sm font-semibold text-red-500">{{ verifyBankTransferPaymentAccountPayload.account_number
                    }}
                </p>
        ...
        <div class="flex flex-col justify-end max-w-full gap-x-5 gap-y-2 lg:flex-row">
            <Button severity="secondary" class="w-full lg:w-1/2" size="small" label="GO BACK"
                @click="paymentStore.previousStep()" />

            <Button size="small" :loading="isSumbittingFormsLoading" class="w-full lg:w-1/2" label="COMPLETE PAYMENT"
...
const isSumbittingFormsLoading = ref(false);
...
async function postFieldForSubmission() {
    isSumbittingFormsLoading.value = true;
    ...
    isSumbittingFormsLoading.value = false;
